...
// ListEventsOrdered returns all events ordered by ts_sec ascending (the
// replay order). Events with the same ts_sec are ordered by created_at.
func (q *Queries) ListEventsOrdered(ctx context.Context) ([]model.Event, error) {
	rows, err := q.db.QueryContext(ctx, `
SELECT id,type,satellite_id,payload_json,ts_sec,created_at FROM events ORDER BY ts_sec ASC, created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
